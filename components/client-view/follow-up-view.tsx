...
const FollowUpViewPage = ({ vitalsId, followUpId }: FollowUpViewProps) => {
  const [patientVitals, setPatientVitals] = useState<PatientVitals | null>(
    null
  );
  const [patientFollowUp, setPatientFollowUp] =
    useState<PatientFollowUp | null>(null);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const vitalsData = await getPatientVitalsById(vitalsId);
        const followUpData = await getPatientFollowUpById(followUpId);

        if (vitalsData && followUpData) {
          setPatientVitals(vitalsData);
          setPatientFollowUp(followUpData);
        }
      } catch (error) {
  ...
  if (!patientVitals || !patientFollowUp) {
  ...
  return (
    <div className="grid grid-cols-2 grid-flow-row gap-6 gap-y-8 p-4">
      <div className="col-span-2">
        <Card className="p-4 grid grid-cols-3 grid-flow-row gap-6 drop-shadow-md">
          <div className="flex flex-row 2xl:gap-x-6 gap-x-4 items-center">
            <h1 className="font-medium">Blood Pressure</h1>
          ...
          <div className="flex flex-row 2xl:gap-x-6 gap-x-4 items-center">
            <h1 className="font-medium">Pulse Rate</h1>
            <p className="flex flex-row items-center 2xl:gap-x-2 gap-x-1">
              <FaHeartbeat className="size-4 text-[#2F80ED]" />
              <span className="font-medium">{patientVitals.pulseRate}</span>
              bpm
            </p>
          </div>

          <div className="flex flex-row 2xl:gap-x-6 gap-x-4 items-center">
            <h1 className="font-medium">Body Temperature</h1>
            <p className="flex flex-row items-center 2xl:gap-x-2 gap-x-1">
          ...
          <div className="flex flex-row 2xl:gap-x-6 gap-x-4 items-center">
            <h1 className="font-medium">Oxygen Saturation</h1>
            <p className="flex flex-row items-center 2xl:gap-x-2 gap-x-1">
              <FaFillDrip className="size-4 text-[#2F80ED]" />
              <span className="font-medium">{patientVitals.oxygen}</span>%
            </p>
          </div>
          ...
          <div className="flex flex-row 2xl:gap-x-6 gap-x-4 items-center">
            ...
            <p className="flex flex-row items-center 2xl:gap-x-2 gap-x-1">
              ...
              <span className="font-medium">{patientVitals.weight}</span>
              kg
            </p>
          </div>
        </Card>
        ...
        <div className="grid grid-cols-3 grid-flow-row gap-6 ">
          ...
          <div className="col-span-1 flex flex-row gap-x-4">
            <Card className="col-span-1 flex flex-col gap-2 p-4 drop-shadow-md">
              ...
              <p className="text-wrap">{patientFollowUp.labResults}</p>
            </Card>
          ...
          <div className="col-span-2 grid grid-cols-2 grid-flow-row gap-6">
            <Card className="col-span-1 flex flex-col gap-2 p-4 drop-shadow-md">
              ...
              <p className="text-wrap">{patientFollowUp.so}</p>
            </Card>
            ...
            <Card className="col-span-1 flex flex-col gap-2 p-4 drop-shadow-md">
              ...
              <p className="text-wrap">{patientFollowUp.diagnosis}</p>
            </Card>
            ...
            <Card className="col-span-1 flex flex-col gap-2 p-4 drop-shadow-md">
              ...
              <p className="text-wrap">{patientFollowUp.treatment}</p>
            </Card>
          </div>
        </div>
      </div>
      ...
      <div className="col-span-2">
        {/* <PlanDetailsPage plan={patientFollowUp.plan || ""} recordId={patientFollowUp.id || ""} /> */}
      </div>
    </div>
  );
};
...
// FIX THIIIISSSS MAKE CLIENT SIDE COMPONENTS!!!
